function [z, correspAlpha] = laserSickSimulator_OccGrid(x, occGrid, occGridParams, angularRes, alpha1, alpha2, maxRange, addNoise) 
...
for i=1:nPoints 
    alpha = minAngle + (maxAngle-minAngle) * (i-1) / (nPoints-1);
    cosAngle(i) = cos( alpha + thetaS );
    sinAngle(i) = sin( alpha + thetaS );    
end
...
%Calculate the dimension of the map
imax = size(occGrid,1);
jmax = size(occGrid,2);
...
% Check if the sensor is outside the map's area, or inside wall regions
if ( (i_pose < 1) || (i_pose > imax )...
    || (j_pose < 1) || (j_pose > jmax )  )
    correspAlpha(i) = (alpha1 + angularRes*(i-1))/180*pi;
